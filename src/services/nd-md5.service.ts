...
@Injectable()
export class NdMd5Service {
  private hex_chr:string = "0123456789abcdef";

  private rhex(num) {
    var str = '';
    for (var j = 0; j <= 3; j++) str += this.hex_chr.charAt((num >> (j * 8 + 4)) & 0x0F) + this.hex_chr.charAt((num >> (j * 8)) & 0x0F);
    return str;
  }

  private str2blks_MD5(str) {
    var i;
